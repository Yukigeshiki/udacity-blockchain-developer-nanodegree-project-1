...
class Blockchain {
    ...
    _addBlock(block) {
        ...
        return new Promise(async (resolve, reject) => {
            try {
                const newHeight = (await self.getChainHeight()) + 1;
    ...
    submitStar(address, message, signature, star) {
        ...
        return new Promise(async (resolve, reject) => {
            ...
            if (currentTime - time > 300) {
                reject('More than 5 minutes has elapsed');
            }
            if (!bitcoinMessage.verify(message, address, signature)) {
                reject('Verification failed');
            }
            const block = new BlockClass.Block({ data: { address, star } });
            await this._addBlock(block);
            resolve(block);
        });
    }
    ...
    getStarsByWalletAddress(address) {
        ...
        const stars = [];
        return new Promise((resolve, reject) => {
            self.chain.slice(1, self.chain.length).forEach(async (block) => {
                const data = (await block.getBData()).data;
    ...
    validateChain() {
        ...
        let errorLog = [];
        return new Promise(async (resolve, reject) => {
            for (const block of self.chain.slice(1, self.chain.length)) {
                const validate = await block.validate();
                if (!validate) {
                    errorLog.push(`Block at height: ${block.height} did not pass validation`);
                }
                if (
                    block.height > 1 &&
                    ...
                    errorLog.push(
                        `Block at height: ${block.height} does not contain the correct hash for the previous block`
                    );
                }
            }
            resolve(errorLog);
